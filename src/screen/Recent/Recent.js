...
import { View, Text, Alert, FlatList, ImageBackground, TouchableHighlight, StyleSheet, ActivityIndicator, TouchableOpacity } from 'react-native';
...
class Recent extends Component {
    constructor(props) {
        ...
        this.pushScreenDetail = this.pushScreenDetail.bind(this);
    }
    ...
    initFirstLoading() {
        this.loading = true;
        axios.get('http://demo2.younetco.com/ctadlib/behereandbeyond/wp-json/wp/v2/videos', {
            params: {
    ...
    renderItem = ({ item }) => (
        <TouchableOpacity onPress={() => this.pushScreenDetail(item)}>
        ...
        return (
            <View style={{ flex: 1 }}>
                <View style={{ flex: 1, flexDirection: 'column' }}>
                    {
                        !this.state.videos.length && !this.canLoadMore && !this.state.show_loading && (<Text style={{ textAlign: 'center', paddingTop: 5 }}>No more items</Text>)
                    }
                    <FlatList
